(function() {
	...
	var messages = {
		...
		'checkout-success': {
			...
			orderSuccessBody:       `주문해 주셔서 감사합니다. 이 대표는 이날 페이스북 글을 통해 "참담함을 금할 수 없다. 우리 사회가 어쩌다 여기까지 왔는지 모르겠다"면서 이같이 밝혔다.

이는 지난 15일 택시기사 안모 씨의 분신을 계기로 실시간 차량공유서비스 타다의 퇴출을 요구하는 택시업계를 정면 비판한 것으로 해석된다.

그는 안 씨의 분신에 대해 "돌아가신 저희 아버지뻘인 76세의 개인택시 기사가 그런 결정을 하기까지 얼마나 두려움이 컸을까 생각하면 안타깝고 미안하기 그지없다"면서도 "누가 근거 없는 두려움을 그렇게 만들어냈고 어떤 실질적 피해가 있었길래 목숨까지 내던졌을까 생각하면 답답하기 그지없다"고 말했다.`,
			orderInfo:              '주문 정보',
			orderId:                '주문 번호',
			orderedAt:              '주문 일시',
